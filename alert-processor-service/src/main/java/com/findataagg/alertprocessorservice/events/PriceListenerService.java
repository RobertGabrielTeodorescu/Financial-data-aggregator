...
public class PriceListenerService {
    ...
    public PriceListenerService(QuoteRepository quoteRepository, TradeRepository tradeRepository, AlertRuleService alertRuleService, AlertEvaluationService alertEvaluationService, UpdatePublishingService updatePublishingService) {
    ...
    public void handleTradeUpdate(PriceUpdate priceUpdate) {
        ...
        if (!activeRules.isEmpty()) {
            ...
            triggeredRules.forEach(rule -> {
                ...
                AlertTriggeredEvent event = new AlertTriggeredEvent(rule.getId(), rule.getSymbol(), rule.getConditionType().toString(), rule.getValue(), priceUpdate.price(), rule.getNotes());
